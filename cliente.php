...
<?php
	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['nome'])) && (!empty($_POST['email'])) && (!empty($_POST['telefone'])) && (!empty($_POST['cpf'])) && (!empty($_POST['data_nasc'])) && (!empty($_POST['rua'])) && (!empty($_POST['bairro'])) && (!empty($_POST['num'])) && (!empty($_POST['cidade'])) && (!empty($_POST['estado']))){
    		$nome = $_POST['nome'];
    		$email = $_POST['email'];
        ...
        $estado = $_POST['estado'];
    		...
    		$sql = "INSERT INTO CLIENTES VALUES('','$nome','$email','$telefone','$cpf','$data_nasc','$rua','$bairro','$num','$cidade','$estado');";

    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="clientes.php">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="clientes.php">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM CLIENTES WHERE ID_cliente=$codigo";
	...
	elseif ($operacao=='editar'){
		...
		$sql = "UPDATE CLIENTES SET nome = '$novonome', email = '$novoemail', telefone = '$novotelefone', cpf = '$novocpf', data_nasc = '$novadata_nasc', rua = '$novarua', bairro = '$novobairro', num = '$novonum', cidade = '$novacidade', estado = '$novoestado'  WHERE ID_cliente = $codigo";


		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="clientes.php">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="clientes.php">Voltar</a></p>';
		}
	} 
	else{
        ...
        echo "<thead><tr><th>ID</th><th>Nome</th><th>E-mail</th><th>Telefone</th><th>CPF</th><th>Data Nasc</th><th>Rua</th><th>Bairro</th><th>Nº</th><th>Cidade</th><th>Estado</th></thead>";
        echo '<tbody>';
