...
<body>
  ...
  <body>
	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['exame'])) && (!empty($_POST['preco'])) && (!empty($_POST['funcionario']))){
    		$exame = $_POST['exame'];
    		$preco = $_POST['preco'];
        $observacoes = $_POST['observacoes'];
        $funcionario = $_POST['funcionario'];
    		$sql = "INSERT INTO EXAMES VALUES('','$exame','$preco','$observacoes','$funcionario');";
    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="exames.php">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="exames.php">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM EXAMES WHERE ID_exame=$codigo";
	...
	elseif ($operacao=='editar'){
		...
		$novoexame = $_POST['exame'];
		$novopreco = $_POST['preco'];
    $novaobservacoes = $_POST['observacoes'];
		$novofuncionario = $_POST['funcionario'];

		$sql = "UPDATE EXAMES SET exame = '$novoexame', preco = '$novopreco', observacoes = '$novaobservacoes' funcionario = '$novofuncionario'  WHERE ID_exame = $codigo";
	
		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="exames.php">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="exames.php">Voltar</a></p>';
		}
	} 
	else{
		...
		echo '<h1 class="text-center">Lista de Exames</h1>';
		echo '<table class="table table-striped">';  
        echo "<thead><tr><th>ID</th><th>Exame</th><th>Preço</th><th>Observações</th><th>Funcionário</th>";
        echo '<tbody>';
