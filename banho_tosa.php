...
<?php
	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['procedimento'])) && (!empty($_POST['preco'])) && (!empty($_POST['funcionario']))){
    		$procedimento = $_POST['procedimento'];
    		$preco = $_POST['preco'];
        	$funcionario = $_POST['funcionario'];

    		$sql = "INSERT INTO BANHO_TOSA VALUES('','$procedimento','$preco','$funcionario');";
			
    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="banhos_tosas.php">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="banhos_tosas.php">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM BANHO_TOSA WHERE ID_banho_tosa=$codigo";
	...
	elseif ($operacao=='editar'){
		$codigo = $_POST['ID_banho_tosa'];
		$novoprocedimento = $_POST['procedimento'];
		$novopreco = $_POST['preco'];
		$novofuncionario = $_POST['funcionario'];

		$sql = "UPDATE BANHO_TOSA SET procedimento = '$novoprocedimento', preco = '$novopreco', funcionario = '$novofuncionario'  WHERE ID_banho_tosa = $codigo";
	
		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="banhos_tosas.php">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="banhos_tosas.php">Voltar</a></p>';
		}
	} 
	else{
		$sql = "SELECT * FROM BANHO_TOSA";
		...
		echo '<h1 class="text-center">Lista de Banhos e Tosas</h1>';
		echo '<table class="table table-striped">';  
        echo "<thead><tr><th>ID</th><th>Procedimento</th><th>Preço</th><th>Funcionário</th>";
        echo '<tbody>';
