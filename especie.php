	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['especie']))){
    		...
    		$sql = "INSERT INTO ESPECIES VALUES('','$especie');";
    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="especies.php">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="especies.php
">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM ESPECIES WHERE ID_especie=$codigo";
	...
	elseif ($operacao=='editar'){
		$codigo = $_POST['ID_especie'];
		$novaespecie = $_POST['especie'];

		$sql = "UPDATE ESPECIES SET especie = '$novaespecie' WHERE ID_especie = $codigo";

		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="especies.php">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="especies.php">Voltar</a></p>';
		}
	} 
		...
		echo '<h1 class="text-center">Lista de Espécies:</h1>';
		echo '<table class="table table-striped">';  
      	echo "<thead><tr><th>ID</th><th>Espécies</th>";
      	echo '<tbody>';
