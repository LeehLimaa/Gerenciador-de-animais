...
<?php
	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['medicamento'])) && (!empty($_POST['marca'])) && (!empty($_POST['modo_uso'])) && (!empty($_POST['descricao'])) && (!empty($_POST['preco'])) && (!empty($_POST['funcionario']))){
    	...
    	$sql = "INSERT INTO MEDICAMENTOS VALUES('','$medicamento','$marca','$modo_uso','$descricao','$preco','$funcionario');";

    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="medicamentos.php
          ">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="medicamentos.php
            ">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM MEDICAMENTOS WHERE ID_medicamentos=$codigo";
	...
	elseif ($operacao=='editar'){
		$codigo = $_POST['ID_medicamentos'];
		$novomedicamento = $_POST['medicamento'];
		$novamarca = $_POST['marca'];
    $novomodo_uso = $_POST['modo_uso'];
		$novadescricao = $_POST['descricao'];
    $novopreco = $_POST['preco'];
		$novofuncionario = $_POST['funcionario'];

		$sql = "UPDATE MEDICAMENTOS SET medicamento = '$novomedicamento', marca = '$novamarca', modo_uso = '$novomodo_uso', descricao = '$novadescricao', preco = '$novopreco', funcionario = '$novofuncionario'  WHERE ID_medicamentos = $codigo";
	
		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="medicamentos.php
      ">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="medicamentos.php
      ">Voltar</a></p>';
		}
	} 
	else{
		...
		echo '<h1 class="text-center">Lista de Medicamentos</h1>';
		echo '<table class="table table-striped">';  
        echo "<thead><tr><th>ID</th><th>medicamento</th><th>Marca</th><th>Mode de Uso</th><th>Descrição</th><th>R$</th><th>Funcionário</th>";
        echo '<tbody>';
