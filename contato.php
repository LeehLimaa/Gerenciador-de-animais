...
<?php
  	...
  	if($operacao=='incluir'){
  		if((!empty($_POST['nome'])) && (!empty($_POST['email'])) && (!empty($_POST['mensagem']))){
    		...
    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Enviado!</p>';
      		echo '<p class="text-center"><a href="contatos.php
          ">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="contatos.php
            ">Voltar</a></p>';
    		}
  		}
  		else{
    		echo '<p class="text-center">Não enviada.<br/>Favor preencher todos os campos.</p>';
    		echo '<p class="text-center"><a href="contatos.php
        ">Voltar</a></p>';  
 		}
 	}
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM CONTATO WHERE ID_contato=$codigo";
		mysqli_query($conexao, $sql);
		//mysqli_affected_row() retorna o número de linhas afetadas
		$linhas = mysqli_affected_rows($conexao);
		if($linhas==1){
			echo '<p class="text-center">Excluido.</p>';
    		echo '<p class="text-center"><a href="contatos.php
        "> Voltar</a></p>';
		}
		else{
	...
	elseif ($operacao=='editar'){
		$codigo = $_POST['ID_contato'];
		$novonome = $_POST['nome'];
		$novoemail = $_POST['email'];
		$novamensagem = $_POST['mensagem'];

		$sql = "UPDATE CONTATO SET nome = '$novonome', email = '$novoemail', mensagem = '$novamensagem' WHERE ID_contato = $codigo";
		
		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="contatos.php
      ">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="contatos.php
      ">Voltar</a></p>';
		}
	} 
	else{
		...
		echo '<h1 class="text-center">Lista de Mensagens Recebidas</h1>';
		echo '<table class="table table-striped">';  
        echo "<thead><tr><th>ID</th><th>Nome</th><th>E-mail</th><th>Mensagem</th>";
        echo '<tbody>';
    ...
    for($i=0; $i<$quantidadeSelecionada; $i++){
        ...
        echo '</tbody></table>';
		echo '<p class="text-center"><a href="contatos.php">Voltar</a></p>';
	}
?>
