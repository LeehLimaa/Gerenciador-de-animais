...
<?php
	...
	$conexao = mysqli_connect('localhost','root','','gerenciador_animais');
  	if($operacao=='incluir'){
  		if((!empty($_POST['tipo'])) && (!empty($_POST['preco'])) && (!empty($_POST['funcionario']))){
    		$tipo = $_POST['tipo'];
        $preco = $_POST['preco'];
        $funcionario = $_POST['funcionario'];
    		$sql = "INSERT INTO CONSULTA VALUES('','$tipo','$preco','$funcionario');";

    		if(mysqli_query($conexao,$sql)){
  	  		echo '<p class="text-center">Cadastrado!</p>';
      		echo '<p class="text-center"><a href="consultas.php">Voltar</a></p>';
    		}
    		else{
      			echo '<p>Erro ao cadastrar no Banco de Dados.</p>';
      			echo '<p><a href="consultas.php">Voltar</a></p>';
    		}
  		}
  		else{
	...
	elseif($operacao=='excluir'){
		$codigo = $_POST['codigo'];
		$sql = "DELETE FROM CONSULTA WHERE ID_consulta=$codigo";
	...
	elseif ($operacao=='editar'){
		...
		$sql = "UPDATE CONSULTA SET tipo = '$novotipo', preco = '$novopreco', funcionario = '$novofuncionario'  WHERE ID_consulta = $codigo";
	
		if(mysqli_query($conexao, $sql)){
			echo '<p class="text-center">Edição realizada!</p>' . '<p class="text-center"><a href="consultas.php">Voltar</a></p>';
		}
		else{
			echo '<p class="text-center">Erro ao editar</p>' . '<p class="text-center"><a href="consultas.php">Voltar</a></p>';
		}
	} 
	else{
		...
		echo '<h1 class="text-center">Lista de Consultas</h1>';
		echo '<table class="table table-striped">';  
        echo "<thead><tr><th>ID</th><th>Tipo</th><th>R$</th><th>Funcionário</th>";
        echo '<tbody>';
